...
# ---------------------------------------------------------------- v8 — site assistant: listing data for the chat (NL + EN)
def _assistant_data():
    import re as _re
    rows = []
    for c in CARDS:
        s = c['url'].rstrip('/').split('/')[-1]
        price = next((x for x in c.get('specs', []) if '€' in x), '')
        nums = [int(_re.sub(r'[.,]', '', n)) for n in _re.findall(r'€\s?([\d.,]*\d)', price)]
        if nums:
            fmt = lambda n: '€' + f'{n:,}'.replace(',', '.')
            price = fmt(nums[0]) if len(set(nums)) == 1 else fmt(nums[0]) + ' – ' + fmt(nums[-1])
        beds = next((x for x in c.get('specs', []) if 'slaapkamer' in x), '')
        rows.append({'t': c['title'], 'te': EN_LISTINGS.get(s, (c['title'],))[0], 'c': c['city'],
                     'p': nums[0] if nums else None, 'p2': nums[-1] if nums else None, 'pr': price,
                     'b': beds, 'i': sm(LPH(s)) if C['listings'][s].get('gallery') else '', 'u': LISTING_FILES[s], 'ue': ALT.get(LISTING_FILES[s], LISTING_FILES[s])})
    js = '/* generated by _build/pages_v8.py — do not edit */\nwindow.PCS_LISTINGS=' + webp(json.dumps(rows, ensure_ascii=False, separators=(',', ':'))) + ';\n'
    (OUT_ASSETS / 'js' / 'assistant-data.js').write_text(js, encoding='utf-8')


_assistant_data()


# ---------------------------------------------------------------- v8 — contact robot 2.0: a consultant robot (headset, suit, brass tie)
# whose eyes follow the pointer; clicking opens the site assistant "Sol" (assets/js/assistant.js)
ROBOT_SVG = (
    '<svg class="bot" viewBox="0 0 64 64" aria-hidden="true">'
    '<path class="bot__suit" d="M6 66c1-12 11-18 26-18s25 6 26 18z" fill="#233759"/>'
    '<path d="M25 48.5 32 58l7-9.5c-2-.4-4.4-.6-7-.6s-5 .2-7 .6z" fill="#FBFBF9"/>'
    '<path d="M31 51h2l1.2 5.4L32 59l-2.2-2.6z" fill="#A9844C"/>'
    '<rect x="28" y="40" width="8" height="8" rx="2" fill="#1B2B47"/>'
    '<g class="bot__ant"><path d="M32 12V6" stroke="#233759" stroke-width="2" stroke-linecap="round"/><circle class="bot__tip" cx="32" cy="5" r="2.8" fill="#A9844C"/></g>'
    '<rect x="14" y="11" width="36" height="31" rx="12" fill="#FBFBF9" stroke="#233759" stroke-width="2"/>'
    '<path class="bot__band" d="M12.5 26a19.5 19.5 0 0 1 39 0" fill="none" stroke="#A9844C" stroke-width="1.8"/>'
    '<rect x="9.5" y="22" width="6" height="11" rx="3" fill="#A9844C"/><rect x="48.5" y="22" width="6" height="11" rx="3" fill="#A9844C"/>'
    '<path d="M12.5 32.5c0 6 4 8.5 10 8.5" fill="none" stroke="#233759" stroke-width="1.6" stroke-linecap="round"/><circle cx="23.5" cy="41" r="1.9" fill="#233759"/>'
    '<rect x="18.5" y="17.5" width="27" height="16" rx="8" fill="#233759"/>'
    '<g class="bot__look"><g class="bot__eyes">'
    '<rect class="bot__eye" x="24" y="21.5" width="4.4" height="7" rx="2.2" fill="#F3DDB0"/><rect class="bot__eye" x="35.6" y="21.5" width="4.4" height="7" rx="2.2" fill="#F3DDB0"/>'
    '<path class="bot__happy" d="M23.8 26.5q2.4-3.2 4.8 0M35.4 26.5q2.4-3.2 4.8 0" fill="none" stroke="#F3DDB0" stroke-width="1.8" stroke-linecap="round"/>'
    '</g></g>'
    '<path class="bot__smile" d="M29 37q3 2 6 0" fill="none" stroke="#A9844C" stroke-width="1.5" stroke-linecap="round"/>'
    '<g class="bot__arm"><path d="M54 66V52" stroke="#233759" stroke-width="6" stroke-linecap="round"/><circle cx="54" cy="47.5" r="4" fill="#FBFBF9" stroke="#233759" stroke-width="1.8"/></g>'
    '</svg>'
)
...
def contact_fab(lang):
    ...
    return (f'\n<button class="cbtn cbtn--bot" type="button" data-chat aria-controls="pcs-chat" aria-expanded="false" '
            f'aria-label="{L("Chat met Sol, onze assistent", "Chat with Sol, our assistant")}">'
            f'<span class="cbtn__hi" aria-hidden="true">{L("Hoi! Ik ben Sol. Vraag het maar!", "Hi! I am Sol. Ask me anything!")}</span>'
            f'<span class="cbtn__av">{ROBOT_SVG}</span><span class="cbtn__t">{L("Chat met Sol", "Chat with Sol")}</span></button>\n')
